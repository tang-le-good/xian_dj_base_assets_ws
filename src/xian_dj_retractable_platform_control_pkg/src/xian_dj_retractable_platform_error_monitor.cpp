...
class XianDjRetractablePlatformErrorMonitor
{
    public:
        XianDjRetractablePlatformErrorMonitor()
        {
            // 创建一个ROS节点句柄
            ros::NodeHandle nh;
            xian_dj_retractable_platform_control_state_sub = nh.subscribe<std_msgs::UInt16>("xian_dj_retractable_platform_control_state_msg", 10, &XianDjRetractablePlatformErrorMonitor::retractable_platform_control_state_callback, this);
            xian_dj_retractable_platform_base_electric_state_sub = nh.subscribe<std_msgs::UInt16>("xian_dj_retractable_platform_base_electric_state_msg", 10, &XianDjRetractablePlatformErrorMonitor::retractable_platform_base_electric_state_callback, this);
        ...
        {
            xian_dj_retractable_platform_control_heart_beat = data->data;
        }

        void retractable_platform_base_electric_state_callback(const std_msgs::UInt16::ConstPtr &data)
        {
            xian_dj_retractable_platform_base_electric_heart_beat = data->data;
        }
        ...
        void m_timer_control_func(const ros::WallTimerEvent& event)
        {
            ...
            if(xian_dj_retractable_platform_control_node_restart_flag > time_out)
            {
                xian_dj_retractable_platform_control_error = 1;
            }
            else
            {
                xian_dj_retractable_platform_control_error = 0;
            }
            ...
            if(xian_dj_retractable_platform_base_electric_node_restart_flag > time_out)
            {
                xian_dj_retractable_platform_base_electric_error = 1;
            }
            else
            {
                xian_dj_retractable_platform_base_electric_error = 0;
            }

            error_sum = xian_dj_retractable_platform_control_error + xian_dj_retractable_platform_base_electric_error;
            ...
            std::string pub_sting = std::to_string(xian_dj_retractable_platform_error_code) 
                                    + "   " + std::to_string(xian_dj_retractable_platform_control_error)
                                    + "   " + std::to_string(xian_dj_retractable_platform_base_electric_error);                     
            pub_msg.data = pub_sting.c_str();
            xian_dj_retractable_platform_error_monitor_pub.publish(pub_msg);
        }
    private:

        ros::Subscriber xian_dj_retractable_platform_control_state_sub;
        ros::Subscriber xian_dj_retractable_platform_base_electric_state_sub; 
        ros::Publisher xian_dj_retractable_platform_error_monitor_pub; 
...
int main(int argc, char** argv)
{
    ...
    spinner.start();
    xian_dj_retractable_platform_error_monitor.m_timer_control = nh_2.createWallTimer(ros::WallDuration(1), &XianDjRetractablePlatformErrorMonitor::m_timer_control_func, &xian_dj_retractable_platform_error_monitor);
    ros::waitForShutdown();
